...
async def start(message:types.Message):
    if message.chat.type == "private":
        ...
        if user_dat.registered:
            await bot.send_message(message.chat.id,"Subscription information",parse_mode="HTML",reply_markup=await main_buttons(user_dat))
        else:
            try:
...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Back :right_arrow_curving_left:":
        await bot.reset_data(m.from_user.id)
        await bot.delete_state(m.from_user.id)
        await bot.send_message(m.from_user.id,"brought you back!",reply_markup=await buttons.admin_buttons())
        return
    if e.demojize(m.text) == "Add time":
        await bot.set_state(m.from_user.id,MyStates.UserAddTimeDays)
        Butt_skip = types.ReplyKeyboardMarkup(resize_keyboard=True)
        Butt_skip.add(types.KeyboardButton(e.emojize(f"Skip :next_track_button:")))
        await bot.send_message(m.from_user.id,"Enter how many days you want to add:",reply_markup=Butt_skip)
        return
    if e.demojize(m.text) == "Reset time":
        await bot.set_state(m.from_user.id,MyStates.editUserResetTime)
        Butt_skip = types.ReplyKeyboardMarkup(resize_keyboard=True)
        Butt_skip.add(types.KeyboardButton(e.emojize(f"Yes")))
        Butt_skip.add(types.KeyboardButton(e.emojize(f"No")))
        await bot.send_message(m.from_user.id,"Are you sure you want to reset the time for this user? ?",reply_markup=Butt_skip)
        return
...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Yes":
        ...
        await db.commit()
        await bot.send_message(m.from_user.id,"Time reset!")

    async with bot.retrieve_data(m.from_user.id) as data:
        usertgid = data['usertgid']
    user_dat = await User.GetInfo(usertgid)
    readymes = f"User: <b>{str(user_dat.fullname)}</b> ({str(user_dat.username)})\nTG-id: <code>{str(user_dat.tgid)}</code>\n\n"

    if int(user_dat.subscription) > int(time.time()):
        readymes += f"Signature: to <b>{datetime.utcfromtimestamp(int(user_dat.subscription)+CONFIG['UTC_time']*3600).strftime('%d.%m.%Y %H:%M')}</b> :check_mark_button:"
    else:
        readymes += f"Subscription: expired <b>{datetime.utcfromtimestamp(int(user_dat.subscription)+CONFIG['UTC_time']*3600).strftime('%d.%m.%Y %H:%M')}</b> :cross_mark:"
...
async def Work_with_Message(m: types.Message):
    if e.demojize(m.text) == "Skip :next_track_button:":
        days=0
    else:
        try:
            days=int(m.text)
        except:
            await bot.send_message(m.from_user.id,"There must be a number!\try again.")
            return
        if days<0:
            await bot.send_message(m.from_user.id, "Must not be a negative number!\try again.")
            return
    ...
    Butt_skip = types.ReplyKeyboardMarkup(resize_keyboard=True)
    Butt_skip.add(types.KeyboardButton(e.emojize(f"Skip :next_track_button:")))
    await bot.send_message(m.from_user.id, "Enter how many hours you want to add:", reply_markup=Butt_skip)
...
async def Work_with_Message(m: types.Message):
    ...
    Butt_skip = types.ReplyKeyboardMarkup(resize_keyboard=True)
    Butt_skip.add(types.KeyboardButton(e.emojize(f"Skip :next_track_button:")))
    await bot.send_message(m.from_user.id, "Enter how many minutes you want to add:", reply_markup=Butt_skip)
...
async def Work_with_Message(m: types.Message):
    ...
    await bot.send_message(m.from_user.id, f"To the user {str(tgid)} will be added:\n\n Days: {str(days)}\n Times: {str(hours)}\n minute: {str(minutes)}\n\nThat's right ?", reply_markup=Butt_skip)
...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Yes":
        ...
        await bot.send_message(m.from_user.id, e.emojize("Time added to user!"), parse_mode="HTML")
    ...
    if int(user_dat.subscription) > int(time.time()):
        ...
        readymes += f"Signature: ended <b>{datetime.utcfromtimestamp(int(user_dat.subscription)+CONFIG['UTC_time']*3600).strftime('%d.%m.%Y %H:%M')}</b> :cross_mark:"
    await bot.set_state(m.from_user.id, MyStates.editUser)
...
async def Work_with_Message(m: types.Message):
    ...
    try:
        ...
        await bot.send_message(m.from_user.id,"Incorrect Id!",reply_markup=await buttons.admin_buttons())
        return
    user_dat = await User.GetInfo(user_id)
    if not user_dat.registered:
        await bot.send_message(m.from_user.id, "This user does not exist!", reply_markup=await buttons.admin_buttons())
        return
...
async def Work_with_Message(m: types.Message):
    if e.demojize(m.text) == "Back :right_arrow_curving_left:":
        await bot.delete_state(m.from_user.id)
        await bot.send_message(m.from_user.id,"go you back!",reply_markup=await buttons.admin_buttons())
        return
    ...
    if set(m.text) <= set(string.ascii_letters+string.digits):
        ...
        await bot.send_message(m.from_user.id,
                               "User added!", reply_markup=await buttons.admin_buttons_static_users())
    else:
        await bot.send_message(m.from_user.id, "Only Latin characters and Persian numerals can be used!\Try again.")
        return
...
async def Work_with_Message(m: types.Message):
    ...
    if m.from_user.id==CONFIG["admin_tg_id"]:
        if e.demojize(m.text) == "Admin panel :smiling_face_with_sunglasses:":
            await bot.send_message(m.from_user.id,"Admin panel",reply_markup=await buttons.admin_buttons())
            return
        if e.demojize(m.text) == "Main menu :right_arrow_curving_left:":
            await bot.send_message(m.from_user.id, e.emojize("Admin panel :smiling_face_with_sunglasses:"), reply_markup=await main_buttons(user_dat))
            return
        if e.demojize(m.text) == "Withdraw users :bust_in_silhouette:":
            await bot.send_message(m.from_user.id, e.emojize("Select which users you want to display."),
        ...
        if e.demojize(m.text) == "All users":
        ...
        if e.demojize(m.text) == "Subscribed users":
            allusers=await user_dat.GetAllUsersWithSub()
            readymass = []
            readymes=""
            if len(allusers)==0:
                await bot.send_message(m.from_user.id, e.emojize("There are no users with a subscription!"), reply_markup=await buttons.admin_buttons(),parse_mode="HTML")
                return
            for i in allusers:
        ...
        if e.demojize(m.text) == "List static users":
            db = await aiosqlite.connect(DBCONNECT)
            ...
            if len(all_staticusers)==0:
                await bot.send_message(m.from_user.id,"There are no static users!")
                return
            for i in all_staticusers:
                Butt_delete_account = types.InlineKeyboardMarkup()
                Butt_delete_account.add(types.InlineKeyboardButton(e.emojize("Delete user :cross_mark:"), callback_data=f'DELETE:{str(i[0])}'))
                ...
                await bot.send_document(chat_id=m.chat.id, document=config,
                                        ...
                                        caption=f"User: <code>{str(i[1])}</code>", parse_mode="HTML",
                                        reply_markup=Butt_delete_account)

            return

        if e.demojize(m.text) =="Edit user по id :pencil:":
            await bot.send_message(m.from_user.id,"Enter Telegram user ID:",reply_markup=types.ReplyKeyboardRemove())
            await bot.set_state(m.from_user.id,MyStates.findUserViaId)
            return

        if e.demojize(m.text) =="Static users":
            await bot.send_message(m.from_user.id,"Выберите пункт меню:",reply_markup=await buttons.admin_buttons_static_users())
            return

        if e.demojize(m.text) =="Add user :plus:":
            await bot.send_message(m.from_user.id,"Enter a name for the new user!\nOnly Latin characters and Persian numerals can be used.",reply_markup=await buttons.admin_buttons_back())
            await bot.set_state(m.from_user.id,MyStates.AdminNewUser)
            return

    if e.demojize(m.text) == "Extend :money_bag:":
        ...
        if True:
            ...
            await bot.send_message(m.chat.id,
                                   "<b>You can pay using a Bank card!</b>\n\Select how many months you would like to purchase a subscription for:",
    ...
    if e.demojize(m.text) == "Как подключить :gear:":
        if user_dat.trial_subscription == False:
            ...
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("For iPhone"), url="https://telegra.ph/Gajd-na-ustanovku-WireGuard-01-16"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("For Android"), url="https://telegra.ph/Gajd-na-ustanovku-WireGuard-Android-01-16"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Check VPN"),
                                           url="https://2ip.ru/"))
            ...
            await bot.send_message(chat_id=m.chat.id,text="First you need to buy a subscription!")
...
async def Buy_month(call: types.CallbackQuery):
    ...
    if payment_info is None:
        ...
        #                     comment=f"Payment VPN на {Month_count} мес. для пользователя {call.from_user.id}")
        # urltopay=CONFIG["url_redirect_to_pay"]+str(new_bill.pay_url)[-36:]
        # bill_id = new_bill.bill_id
        await bot.delete_message(call.message.chat.id, call.message.id)
        bill = await bot.send_invoice(call.message.chat.id,f"Payment VPN",f"VPN on {str(Month_count)} мес.",call.data,currency="toman",prices=[types.LabeledPrice(f"VPN on {str(Month_count)} months.", Month_count*CONFIG['one_month_cost']*100)],provider_token=CONFIG["tg_shop_token"])
        #await user_dat.NewPay(bill.,Month_count*CONFIG['one_month_cost'],Month_count*2592000,call.message.id)

        # Butt_payment = types.InlineKeyboardMarkup()
        # Butt_payment.add(
        #     types.InlineKeyboardButton(e.emojize("Pay :money_bag:"), url=urltopay))
        # Butt_payment.add(
        #     types.InlineKeyboardButton(e.emojize("Cancel payment :cross_mark:"), callback_data=f'Cancel:' + str(user_dat.tgid)))
        # await bot.edit_message_text(chat_id=call.from_user.id,message_id=call.message.id,text=f"<b>Payment: VPN on {str(Month_count)} months.\n\nPayment amount: <code>{str(Month_count*CONFIG['one_month_cost'])} ₽</code></b>\nPay your bill within 45 minutes!",parse_mode="HTML",reply_markup=Butt_payment)
...
#         await bot.edit_message_text(chat_id=call.from_user.id,message_id=call.message.id,text="Payment cancelled!",reply_markup=None)
#
#
#     await bot.answer_callback_query(call.id)
...
async def AddTimeToUser(tgid,timetoadd):
    ...
    if int(userdat.subscription) < int(time.time()):
        ...
        await bot.send_message(userdat.tgid, e.emojize( 'The login details have been updated, download the new authorization file through the section "How to connect :gear:"'))
    else:
    ...
    if int(passdat) >= timenow:
        Butt_main.add(
            types.KeyboardButton(e.emojize(f":green_circle: To: {dateto} МСК:green_circle:")))

    Butt_main.add(types.KeyboardButton(e.emojize(f"Extend :money_bag:")),
                  types.KeyboardButton(e.emojize(f"How to connect :gear:")))
...
async def DeleteUserYesOrNo(call: types.CallbackQuery):
    ...
    if staticuser[0]!=int(idstatic):
        await bot.answer_callback_query(call.id, "User has already been deleted!")
        return

    if "DELETE:" in call.data:
        Butt_delete_account = types.InlineKeyboardMarkup()
        Butt_delete_account.add(types.InlineKeyboardButton(e.emojize("Delete!"),callback_data=f'DELETYES:{str(staticuser[0])}'),types.InlineKeyboardButton(e.emojize("No"),callback_data=f'DELETNO:{str(staticuser[0])}'))
    ...
    if "DELETYES:" in call.data:
        ...
        await bot.answer_callback_query(call.id,"User deleted!")
        return
    if "DELETNO:" in call.data:
        Butt_delete_account = types.InlineKeyboardMarkup()
        Butt_delete_account.add(types.InlineKeyboardButton(e.emojize("Delete user :cross_mark:"),
...
async def checkout(pre_checkout_query):
    ...
    if month*100*CONFIG['one_month_cost']!=pre_checkout_query.total_amount:
        await bot.answer_pre_checkout_query(pre_checkout_query.id, ok=False,
                                            error_message="Can't buy at old price!")
        await bot.send_message(pre_checkout_query.from_user.id,"<b>The price has changed! Cannot be purchased at the old price!</b>",parse_mode="HTML")
    else:
        ...
        try:
            ...
            for i in log:
                ...
                if remained_time<=0 and i[3]==False:
                    ...
                    Butt_main = types.ReplyKeyboardMarkup(resize_keyboard=True)
                    Butt_main.add(
                            types.KeyboardButton(e.emojize(f":red_circle: Ended: {dateto} МСК:red_circle:")))
                    Butt_main.add(types.KeyboardButton(e.emojize(f"Extend :money_bag:")),
                                  types.KeyboardButton(e.emojize(f"How to connect :gear:")))
                    BotChecking = TeleBot(BOTAPIKEY)
                    BotChecking.send_message(i['tgid'],
