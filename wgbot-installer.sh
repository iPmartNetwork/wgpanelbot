	...
	if [[ -e /etc/debian_version ]]; then
		...
		if [[ ${ID} == "debian" || ${ID} == "raspbian" ]]; then
			if [[ ${VERSION_ID} -lt 10 ]]; then
				echo "Your Debian version (${VERSION_ID}) not supported. Please use Debian 10 Buster or lower"
				exit 1
			fi
...
function installQuestions() {
	...
	read -rp "Enter public IPv4 adress: " -e -i "${SERVER_PUB_IP}" SERVER_PUB_IP

	# Detect public interface and pre-fill for the user
	SERVER_NIC="$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1)"
	until [[ ${SERVER_PUB_NIC} =~ ^[a-zA-Z0-9_]+$ ]]; do
		read -rp "Enter network interface: " -e -i "${SERVER_NIC}" SERVER_PUB_NIC
	done
	...
	until [[ ${SERVER_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_PORT}" -ge 1 ] && [ "${SERVER_PORT}" -le 65535 ]; do
		read -rp "Select the port to use WireGuard [1-65535]: " -e -i "${RANDOM_PORT}" SERVER_PORT
	done

	# Cloudflare DNS by default
	until [[ ${CLIENT_DNS_1} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "Select the first DNS server that clients will use: " -e -i 1.1.1.1 CLIENT_DNS_1
	done
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		read -rp "Select the second DNS server that clients will use (opcional'no): " -e -i 1.0.0.1 CLIENT_DNS_2
		if [[ ${CLIENT_DNS_2} == "" ]]; then
			CLIENT_DNS_2="${CLIENT_DNS_1}"
		fi
	done
	read -rp "Do you want to install the Telegram bot immediately?(1 - Da, 0 - Net): " -e -i "1" BOT_AUTO_INSTALL
	
	if [[ ${BOT_AUTO_INSTALL} == '1' ]]; then
		read -rp "Enter the API key for your Telegram bot: " -e API_TOKEN_BOT
		read -rp "Enter the key for your payment system (If left blank, users will not be able to make online payments): " -e API_PAYMENT_BOT
...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		...
		if [[ ${BOT_AUTO_INSTALL} == '1' ]]; then
			...
			wget https://codeload.github.com/iPmartNetwork/wgpanelbot/zip/refs/heads/main/wgpanelbot-main.zip
			unzip wgpanelbot-main.zip
			rm wgpanelbot-main.zip
...
Restart=always
RestartSec=15
WorkingDirectory=$(pwd)/wgpanelbot
ExecStart=/usr/bin/python3 $(pwd)/wgpanelbot/main.py
User=root
